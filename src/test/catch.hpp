...
#  define INTERNAL_CATCH_TEST_CASE_METHOD(ClassName, ...)                     \
    ...
    Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME(autoRegistrar)(                 \
        ...
        #ClassName, Catch::NameAndDesc(__VA_ARGS__),                          \
...
// So the compare overloads can be operator agnostic we convey the operator as
// a template enum, which is used to specialise an Evaluator for doing the
// comparison.
...
class TestSpec {
  ...
  public:
    ...
    virtual bool matches(TestCaseInfo const &testCase) const {
      switch (m_wildcard) {
      case NoWildcard: return m_name == toLower(testCase.name);
      case WildcardAtStart: return endsWith(toLower(testCase.name), m_name);
      case WildcardAtEnd: return startsWith(toLower(testCase.name), m_name);
      case WildcardAtBothEnds: return contains(toLower(testCase.name), m_name);
      }
...
template <typename T> class ReporterRegistrar {
  class ReporterFactory: public IReporterFactory {
    ...
    // this. In fact, ideally, please contact me anyway to let me know you've
    // hit this - as I have no idea who is actually using custom reporters at
    // all (possibly no-one!). The new interface is designed to minimise
    // exposure to interface changes in the future.
    virtual IStreamingReporter *create(ReporterConfig const &config) const {
      return new T(config);
    }
...
#    define FAIL(...)                                                         \
      INTERNAL_CATCH_MSG(Catch::ResultWas::ExplicitFailure,                   \
                         Catch::ResultDisposition::Normal, "FAIL",            \
                         __VA_ARGS__)
#    define SUCCEED(...)                                                      \
      INTERNAL_CATCH_MSG(Catch::ResultWas::Ok,                                \
                         Catch::ResultDisposition::ContinueOnFailure,         \
